CREATE OR REPLACE PROCEDURE `CRG_DIM_CONTACTO`(VAR_SCHEMA varchar(10) CHARACTER SET utf8 COLLATE utf8_general_ci NULL) RETURNS void AS
/***********************************************************************************************************************
...
START TRANSACTION;
	...
	VAR_SQL=CONCAT('DROP TABLE IF EXISTS ',VAR_SCHEMA,'.TMP_CONTACTO');
	 EXECUTE IMMEDIATE VAR_SQL;
	COMMIT;	

-- GUARDA EN UNA TABLA TEMPORAL LOS REGISTROS ACTUALIZADOS
	VAR_SQL=CONCAT('CREATE TABLE ',VAR_SCHEMA,'.TMP_CONTACTO  
	SELECT  R.SK_CONTACTO,R.ID_CRM,R.COD_EMPRESA,R.TIPO_CLIENTE
	...
	LEFT JOIN (SELECT DISTINCT
				...
				, CC.id_c AS ID_CRM
 				, "01" AS COD_EMPRESA
 				, CC.tipo_cliente_c AS TIPO_CLIENTE
				...
				WHERE TC.id IS NOT NULL AND C.id IS NOT NULL AND CC.id_c IS NOT NULL) P 
	ON R.ID_CRM = P.ID_CRM AND R.COD_EMPRESA = P.COD_EMPRESA AND R.CEDULA = P.CEDULA			
	');
	EXECUTE IMMEDIATE VAR_SQL;
	COMMIT;
	...
	VAR_SQL=CONCAT('INSERT INTO DWH.DIM_CONTACTO
	  (SK_CONTACTO,ID_CRM,COD_CLIENTE,COD_EMPRESA,TIPO_CLIENTE,NOMBRE,DIRECCION,TELEFONO,TELEFONO2,EMAIL1,CEDULA,PAIS,PROVINCIA,FECHA_CARGA)
          SELECT  A.SK_CONTACTO,A.ID_CRM,A.COD_CLIENTE,A.COD_EMPRESA,A.TIPO_CLIENTE,A.NOMBRE,A.DIRECCION,A.TELEFONO,A.TELEFONO2,A.EMAIL1,A.CEDULA,A.PAIS,A.PROVINCIA,A.FECHA_CARGA
	  FROM ',VAR_SCHEMA,'.TMP_CONTACTO A');
	  EXECUTE IMMEDIATE VAR_SQL;
	COMMIT;
	...
	VAR_SQL=CONCAT('CREATE TABLE ',VAR_SCHEMA,'.TMP_CONTACTO
		SELECT A.ID_CRM,A.COD_CLIENTE,A.COD_EMPRESA,A.TIPO_CLIENTE,A.NOMBRE,A.DIRECCION,A.TELEFONO,A.TELEFONO2,A.EMAIL1,A.CEDULA,A.PAIS,A.PROVINCIA
			FROM (SELECT DISTINCT
				NVL(CC.id_c,"0") AS ID_CRM
				, NVL(CC.id_cliente_c,"0") AS COD_CLIENTE
 				, "01" AS COD_EMPRESA
 				, NVL(CC.tipo_cliente_c,"0") AS TIPO_CLIENTE
 				, CONCAT_WS(" ",C.first_name,C.last_name) AS NOMBRE
 				, CONCAT_WS(" ",CC.calle_principal_c,CC.calle_secundaria_c) AS DIRECCION
 				, C.phone_home AS TELEFONO
 				, C.phone_mobile AS TELEFONO2
				, T.email AS EMAIL1
 				, CC.numero_identificacion_c AS CEDULA
				...
				WHERE TC.id IS NOT NULL AND C.id IS NOT NULL AND CC.id_c IS NOT NULL) A
     ');
	EXECUTE IMMEDIATE VAR_SQL;
	COMMIT;
	...
	VAR_SQL=CONCAT('INSERT INTO DWH.DIM_CONTACTO
		(ID_CRM,COD_CLIENTE,COD_EMPRESA,TIPO_CLIENTE,NOMBRE,DIRECCION,TELEFONO,TELEFONO2,EMAIL1,CEDULA,PAIS,PROVINCIA)
		SELECT A.ID_CRM,A.COD_CLIENTE,A.COD_EMPRESA,A.TIPO_CLIENTE,A.NOMBRE,A.DIRECCION,A.TELEFONO,A.TELEFONO2,A.EMAIL1,A.CEDULA,A.PAIS,A.PROVINCIA 
		FROM ',VAR_SCHEMA,'.TMP_CONTACTO A
		LEFT JOIN DWH.DIM_CONTACTO B ON A.ID_CRM = B.ID_CRM AND A.COD_EMPRESA = B.COD_EMPRESA AND A.CEDULA = B.CEDULA
